...
class RepositoryJsonTest {
    ...
    private RentalTotalRepository rentalTotalRepo;

    @BeforeEach
    void setUp() {
        File customerFile = tempDir.resolve("customers.json").toFile();
        File movieFile = tempDir.resolve("movies.json").toFile();
        File totalsFile = tempDir.resolve("totals.json").toFile();
        ...
        rentalTotalRepo = new RentalTotalRepository(
    ...
    void testRentalTotalRepositorySaveAndRead() throws RentalNegativeDayException, MovieTitleException, CustomerUserNameException {
        ...
        rentalTotalRepo.addItem(totals);

        List<RentalTotal> readBack = rentalTotalRepo.getAllItems();
        assertEquals(1, readBack.size());
        Rental readRental = readBack.get(0).getRental();
        assertEquals(customer.getId(), readRental.getCustomer().getId());
        assertEquals(customer.getUserName(), readRental.getCustomer().getUserName());
        assertEquals(2, readRental.getMovies().size());
    }
}
